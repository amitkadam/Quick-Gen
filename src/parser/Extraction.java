...
public class Extraction {
	...
	Pattern p_opening_cir_brace = Pattern.compile(JavaGrammer.opening_circular_brace);
	Pattern p_closing_cir_brace = Pattern.compile(JavaGrammer.closing_circular_brace);
	...
	public void extractVarDetails(String filePath)
	{
		...
		while(line_num < total_lines)
		{
			...
			if (found_class != true)
			{
				class_name = get_class_name(data, m_class, m_class_name, p_class, p_class_name);
				if (class_name != null)
			...
			else
			{
				...
				if (m_label.find())
				{
					...
					int label_is_function = is_label_function(p_is_fun, m_label.end(0), data);
					if(label_is_function != 0)
					{
						...
						fn.setParameters(get_function_parameters(pos, data, m_label));
	...
	private ArrayList<Statement> processFunctionBody(Matcher m_label) {
		...
		//but line_num is getting incremented in calling function before this call.
		//line_num--;
		...
		Statement s;
		/*	Check if function body curly braces start, if starts on function declaration line, move to next line, 
		 *  if function body 1st statement starts on same line where brace is present, ignore that brace and read code of that line. 
		 */	
		...
		for (int ln_num = fn_start; ln_num < fn_end; ln_num++)
		{
			...
			//System.out.println ("line num is " + ln_num + "fn_start is " + fn_start);
	...
	private String ifHasIfCond_Getcond(String data, int pos) {
		...
		if(m_if_cond.find())
		{
			...
			return data.substring(m_opening_cir_brace.end(), m_closing_cir_brace.end() - 1); 
		}
		return null;
	}

	/*
	 * Given a line number after which block will start,
	 * finds the block start and block end line number
	 * and sets block_start and block_end variables.
	 */
	...
	private ArrayList<String> get_function_parameters(int start_pos, String data, Matcher m_label) {
